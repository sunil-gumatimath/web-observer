...
function buildRows(before: string, after: string): Row[] {
  ...
  while (k < ops.length) {
    const op = ops[k];
    if (op.t === "eq") {
      rows.push({ kind: "equal", left: a[op.i], leftNo: op.i + 1, right: b[op.j], rightNo: op.j + 1 });
      k++;
      continue;
    }
    const dels: number[] = [];
    ...
    for (let x = 0; x < max; x++) {
      ...
      if (hasDel && hasAdd) {
        rows.push({ kind: "replace", left: a[dels[x]], leftNo: dels[x] + 1, right: b[adds[x]], rightNo: adds[x] + 1 });
      } else if (hasDel) {
...
function UnifiedLine({ text, variant }: { text: string; variant: "equal" | "del" | "add" }) {
  const cls =
    variant === "del"
  ...
  return (
    <div className={cn("flex px-3", cls)}>
      <span className="whitespace-pre-wrap break-words">{sign}{text}</span>
    </div>
  );
}
...
export function GithubDiff({
  ...
  if (!hasContent) {
    return <p className="text-sm text-slate-500 dark:text-slate-400">No text content.</p>;
  }

  const added = rows.filter((r) => r.kind === "add" || r.kind === "replace").length;
  const removed = rows.filter((r) => r.kind === "del" || r.kind === "replace").length;
          ...
          <pre className="diff m-0 whitespace-pre-wrap break-words p-3 font-mono text-[13px] leading-5">
            {unifiedDiff ? (
              unifiedDiff.split("\n").map((line, i) => {
                ...
                if (t.startsWith("+") && !t.startsWith("+++")) return <span key={i} className="add block">{line}</span>;
                if (t.startsWith("-") && !t.startsWith("---")) return <span key={i} className="del block">{line}</span>;
      ...
      <div>
        ...
        <CardShell>
          <div className="max-h-[min(70vh,32rem)] overflow-auto py-1 font-mono text-[13px] leading-5">
            {rows.map((r, i) => {
              if (r.kind === "equal") return <UnifiedLine key={i} text={r.left ?? ""} variant="equal" />;
              if (r.kind === "del") return <UnifiedLine key={i} text={r.left ?? ""} variant="del" />;
              if (r.kind === "add") return <UnifiedLine key={i} text={r.right ?? ""} variant="add" />;
              return (
                <div key={i}>
